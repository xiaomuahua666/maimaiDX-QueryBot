...
# ── 群消息 @机器人 判定：@所有人 不唤出，@机器人 才响应 ──

class _FakeMentionId:
    def __init__(self, open_id):
        self.open_id = open_id

class _FakeMention:
    def __init__(self, open_id, key="@_user_1", name=""):
        self.id = _FakeMentionId(open_id)
        self.key = key
        self.name = name

class _FakeMessage:
    def __init__(self, mentions, chat_id="oc_test", message_type="text"):
        self.mentions = mentions
        self.chat_id = chat_id
        self.message_type = message_type
        self.message_id = "om_x"
        self.content = '{"text":"帮助"}'

mention_bot = object.__new__(MODULE.FeishuOpsBot)
mention_bot._bot_open_id = "ou_bot"
# @机器人 -> 响应
assert mention_bot._is_bot_mentioned(
    _FakeMessage([_FakeMention("ou_bot")])
) is True
# @其他人 -> 不响应
assert mention_bot._is_bot_mentioned(
    _FakeMessage([_FakeMention("ou_someone_else")])
) is False
# @所有人（open_id == "all"）-> 不响应
assert mention_bot._is_bot_mentioned(
    _FakeMessage([_FakeMention("all", key="@_all", name="所有人")])
) is False
# @其他人 + @所有人，没有 @机器人 -> 不响应
assert mention_bot._is_bot_mentioned(
    _FakeMessage([_FakeMention("ou_someone_else"), _FakeMention("all", key="@_all")])
) is False
# @其他人 + @机器人 -> 响应
assert mention_bot._is_bot_mentioned(
    _FakeMessage([_FakeMention("ou_someone_else"), _FakeMention("ou_bot", key="@_user_2")])
) is True
# 无 mentions -> 不响应
assert mention_bot._is_bot_mentioned(_FakeMessage([])) is False

# handle_message：群聊 @所有人 不产生回复卡片
hm_bot = object.__new__(MODULE.FeishuOpsBot)
hm_bot._bot_open_id = "ou_bot"
hm_bot.config = MODULE.OpsConfig(
    app_id="cli_x", app_secret="s",
    allowed_chat_ids=frozenset({"oc_test"}),
    admin_open_ids=frozenset({"ou_admin"}),
    super_admin_open_ids=frozenset(),
    admin_api_url="http://127.0.0.1:1", admin_api_token="",
    state_db=Path(tempfile.gettempdir()) / "fob-test-actions.sqlite3",
)
replied = []

def fake_reply(message_id, card):
    replied.append((message_id, card))

hm_bot._reply_card = fake_reply
hm_bot.is_admin = lambda oid: oid == "ou_admin"
...
def _mk_data(mentions, chat_id="oc_test", sender_open_id="ou_admin"):
    msg = _FakeMessage(mentions, chat_id=chat_id)
    return _FakeData(_FakeEvent(msg, _FakeSender(sender_open_id)))

# @所有人 + 合法命令 -> 不应回复
hm_bot.handle_message(_mk_data([_FakeMention("all", key="@_all", name="所有人")]))
assert replied == [], f"@所有人 不应唤出机器人, got {replied}"
# @机器人 + 合法命令 -> 应回复（菜单卡）
hm_bot.handle_message(_mk_data([_FakeMention("ou_bot")]))
assert len(replied) == 1, f"@机器人 应唤出机器人, got {replied}"

print("mention gating checks: OK")
