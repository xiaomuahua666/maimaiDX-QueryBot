...
async def main() -> None:
    ...
    try:
        ...
        assert getattr(payload[0], "type", None) == "markdown"
        payload_content = payload[0].data["markdown"].content
        assert payload_content.startswith(
            '<qqbot-at-user id="guess-user-openid" />\n'
        )
        assert "猜对了！" in payload_content
    finally:
